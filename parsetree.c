...
void printParseTreeNode(parseTreeNode* n)
{
	...
	if(isTerminal(n->symbol) || strcmp(n->symbol, "<var_name>") == 0 || strcmp(n->symbol, "<const>") == 0)
	{
		printf(" %-35s","terminal");
		printf(" %-40s", "#######");
		printf(" %-15s", n->lexeme);
	...
	else
	{
		...
		printf(" %-40s", "Present In Type Expression Table.");
		printf(" %-15s", "#######");
		printf(" %8s", "#######");
